...
#CLIMuu = np.load(basedir + 'u_CLIM.npy')
...
#CLIMvv = np.load(basedir2 + 'v_CLIM.npy')
...
#plt.step(bar_center,h1,color='blue')
#plt.step(bar_center,h2,color='red')
...
d = 10; u = 1200; n = 13;
h1,bar_edges = np.histogram(ENd1,bins=np.linspace(d,u,n),density=True)
#print(h1,np.nanmin(ENd1),np.nanmean(ENd1),np.nanmax(ENd1))
h2,_ = np.histogram(LNd1,bins=np.linspace(d,u,n),density=True)
bar_center = np.asarray([(bar_edges[i] + bar_edges[i+1])/2 for i in np.arange(len(bar_edges)-1)])
ww = np.asarray([bar_edges[i]-bar_edges[i-1] for i in np.arange(1,len(bar_edges))])
...
# EXTREME WIND PROFILES
#ax2 = plt.subplot2grid((1,3),(0,1))
#ax2.plot(windhi[0]-windhi[3],press,color='blue',linewidth=1.25)
#ax2.plot(windhi[1]-windhi[4],press,color='green',linewidth=1.25)
#ax2.plot(windhi[2]-windhi[5],press,color='red',linewidth=1.25)
#ax2.plot(windhi[3],press,color='blue',linewidth=1.25,linestyle='--')
#ax2.plot(windhi[4],press,color='green',linewidth=1.25,linestyle='--')
#ax2.plot(windhi[5],press,color='red',linewidth=1.25,linestyle='--')
#ax2.tick_params(axis='both',labelsize=fs-1)
#plt.ylim([300,1000])
#ax2.invert_yaxis()
#plt.text(0.88,0.92,'(b)',fontsize=13,fontweight='bold',transform=ax2.transAxes)
#plt.xlabel(r'(EN-LN)/LN relative difference in '
#           '\n'
#           'extreme wind speed [m s$^{-1}$]',fontsize=fs)
