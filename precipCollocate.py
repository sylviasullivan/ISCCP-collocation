#!/usr/bin/env mcsplot
# SAVE THE VECTOR OF (1) MCS DEPTH [K] (2) SST IN CONVECTIVE CORE [K] 
...
def precipCollocate5_local(ENSO):
    ...
    DD[:,4]  = CTdata[:,21]      # core lat, 12 = sys lat
    DD[:,5]  = CTdata[:,22]      # core lon, 13 = sys lon
    ...
    for ii in range(len(yrs)):
        ...
        for jj in range(zeit.shape[0]):
            ...
            for kk in range(len(indx)):
                ...
                #lon[lon == 720] = 0
                lon = int(round(subset[kk,5]*2.0))
                lat = int(round(subset[kk,4]*2.0)) + 180
                if lon != 720:
                   pr = prec[jj,lat,lon] 
                
                # calculate the SST from the core lat/lon and time
